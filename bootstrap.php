...
function get_favicon(string $entry): string
{
    ...
    if (!empty($bibleconfig->favicon)) {
        $favicon = 'data:image/x-icon;base64,' . $bibleconfig->favicon;
    } else {
        // Default bible favicon.
...
{
    $bibleconfig = get_config('local_bible');
    if (!empty($bibleconfig->logo)) {
        $logo = 'data:image/x-icon;base64,' . $bibleconfig->logo;
    } else {
        // Default bible logo.
        $manifest = getManifest();
        $src = isDev($entry)
...
function bootstrap(string $entry): string
{
    ...
    if (isDev($entry)) {
        $output .= <<<HTML
            ...
            <script type="module" src="{$host}{$base_dir}/frontend/src/main.jsx"></script>
        HTML;
    } else {
        $output .= "\n" . jsTag($entry)
